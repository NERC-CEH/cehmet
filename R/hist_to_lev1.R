...
process_logger_files <- function(
  ...
  if (check_time) {
    v_fname <- lapply(v_fname, remove_time_errors)
  }
...
subset_by_year <- function(
  ...
  write_out <- function(i_year) {
    ...
    fname <- path(
      ...
      site_id,
      level,
      paste0(site_id, "_BM_dt_", i_year, ".csv")
    )
    fwrite(
      l_lev$dt[year(DATECT) == i_year],
      file = fname,
    ...
    fname <- path(
      ...
      site_id,
      level,
      paste0(site_id, "_BM_qc_", i_year, ".csv")
    )
    fwrite(
      l_lev$dt_qc[year(DATECT) == i_year],
      file = fname,
      dateTimeAs = "write.csv"
    )
    # the lot
    fname <- path(
      ...
      site_id,
      level,
      paste0(site_id, "_BM_", i_year, ".rds")
    )
...
logger_to_lev0 <- function(dt, v_names_for_db, fname_out = NULL) {
  ...
  dt_qc[,
    v_names_for_db[-1] := lapply(.SD, function(x) as.numeric(is.na(x))),
    .SDcols = v_names_for_db[-1]
  ]
...
join_met <- function(l_met1, l_met2) {
  l_met2$dt <- power_full_join(
    l_met1$dt,
    l_met2$dt,
    by = "DATECT",
    conflict = coalesce_yx
  )
  l_met2$dt_qc <- power_full_join(
    l_met1$dt_qc,
    l_met2$dt_qc,
    by = "DATECT",
    conflict = coalesce_yx
