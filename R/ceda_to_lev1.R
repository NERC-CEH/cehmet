ceda_to_lev0 <- function(v_fname, v_names_for_db, df_names, write_csv = FALSE) {
  ...
  if (write_csv) fwrite(dt, file = here("data", "UK-Amo_BM_lev0.csv"))
  return(dt)
}

plot_lev0_vs_era5 <- function(dt, df_era5, v_names = v_names_for_db[-1], avg.time = "1 week") {
  setnames(dt, "DATECT", "date")
  setnames(df_era5, "DATECT", "date")
  if (!is.na(avg.time)) {
    dt      <- as.data.table(timeAverage(dt, avg.time = avg.time, fill = TRUE))
    df_era5 <- as.data.table(timeAverage(df_era5, avg.time = avg.time, fill = TRUE))
  }
  ...
  for (var_name in v_names) {
    ...
    p <- p + geom_line(data = df_era5, aes(date, get(var_name)), colour = "black")
...
convert_lev0_to_lev1 <- function(dt, df_era5, write_csv = FALSE) {
  ...
  l_lev1 <- impute(y = "P_12_1_1", l_met = l_lev1, method = "era5", fit = FALSE, df_era5 = as.data.frame(df_era5), plot_graph = plot_graph)
