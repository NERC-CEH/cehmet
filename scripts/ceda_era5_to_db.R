...
# get names
df_names <- read_excel(
  here("data", "ERA5_to_ICOS.xlsx"),
  sheet = "mainmet_to_era5"
)
...
l_dt[[28]] <- l_dt[[28]][, 1:55]
l_dt[[29]] <- l_dt[[29]][, 1:55]
...
min(df_era5$DATECT)
max(df_era5$DATECT)
min(l_lev1_ceda$df_qc$DATECT)
max(l_lev1_ceda$df_qc$DATECT)
min(l_lev1_thisyear$df_qc$DATECT)
max(l_lev1_thisyear$df_qc$DATECT)
...
min(l_lev1$df_qc$DATECT)
max(l_lev1$df_qc$DATECT)
