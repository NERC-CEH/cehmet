...
dir_in  <- paste0("/gws/nopw/j04/ceh_generic/amo_met/server_mirror/complete/",format(Sys.Date(), "%Y")) # Now looking at the current year folder
...
v_date_to_process <- as.POSIXlt(seq(from = first_date_to_process,
...
for (i_day in 1:length(v_date_to_process)) {
  ...
  for (i_file in 1:length(v_fname_in)) {
    pname_daily <- get_pathname_daily(date_to_process, dir_out,
      station_code, v_logger_id[i_file], v_file_id[i_file], ext)

    file_already_existed <- file.exists(pname_daily)
    print(paste(path_file(pname_daily), "already existed:", file_already_existed))
    ...
    if (!file_already_existed | overwrite == TRUE) {
      ...
      written <- write_daily_file(date_to_process, dir_in, v_fname_tc[i_file],
        pname_daily)
    }
...
#   }
	...
	if ((!file_already_existed & do_upload == TRUE) | force_upload == TRUE) {
      err <- upload_daily_file(pname_daily) # have 3 goes - before I can get this sorted.
	  if(err != 0){ # if it fails then remove the daily file - so it can be caught again the next day(s).
	  unlink(pname_daily)
	  }
    } else {
