...
for (raw_file in raw_files) {
  ...
  colnames(raw_data) <- t(read.table(
    file = raw_file,
    sep = ",",
    skip = 1,
    nrows = 1
  ...
  raw_data$DateTime <- as.POSIXct(
    strptime(as.character(paste(raw_data$TIMESTAMP)), "%Y-%m-%d %H:%M:%S"),
    format = "%Y-%m-%d %H:M:S",
    tz = "GMT"
  )
  ...
  ContTimeStamp <- data.frame(
    "DateTime" = seq(
      from = min(raw_data$DateTime, na.rm = T),
      to = max(raw_data$DateTime, na.rm = T),
      by = time_interval
    )
  )
  raw_data <- merge(
  ...
  if (file.exists(comp_file)) {
    ...
    colnames(comp_data) <- t(read.table(
      file = raw_file,
      sep = ",",
      skip = 1,
      nrows = 1
    ...
    comp_data$DateTime <- as.POSIXct(
      strptime(as.character(paste(comp_data$TIMESTAMP)), "%Y-%m-%d %H:%M:%S"),
      format = "%Y-%m-%d %H:M:S",
      tz = "GMT"
    )
    ...
    ContTimeStamp <- data.frame(
      "DateTime" = seq(
        from = min(comp_data$DateTime, na.rm = T),
        to = max(comp_data$DateTime, na.rm = T),
        by = time_interval
      )
    )
    comp_data <- merge(
    ...
    combined_data$DateTime <- as.POSIXct(
      strptime(
        as.character(paste(combined_data$TIMESTAMP)),
        "%Y-%m-%d %H:%M:%S"
      ),
      format = "%Y-%m-%d %H:M:S",
      tz = "GMT"
    ...
    ContTimeStamp <- data.frame(
      "DateTime" = seq(
        from = min(combined_data$DateTime, na.rm = T),
        to = max(combined_data$DateTime, na.rm = T),
        by = time_interval
      )
    )
    combined_data <- merge(
  ...
  for (year in 2022:as.numeric(current_year)) {
    ...
    if (!dir.exists(paste(CompFileDir, year, sep = "/"))) {
      ...
      Sys.chmod(
        paste(CompFileDir, year, sep = "/"),
        mode = "0775",
        use_umask = FALSE
      ) # set permissions so that anyone in the gws can write to this folder
    }
    ...
    if (year == current_year) {
      ...
      write.table(
        Header1,
        file = paste(CompFileDir, year, file_name, sep = "/"),
        sep = ",",
        col.names = F,
        row.names = F,
        append = F,
